...
extension UITableView: CollectionSkeleton {
    ...
    var skeletonDataSource: SkeletonCollectionDataSource? {
        get { withUnsafePointer(to: &CollectionAssociatedKeys.dummyDataSource) { return ao_get(pkey: $0) as? SkeletonCollectionDataSource } }
        set {
            withUnsafePointer(to: &CollectionAssociatedKeys.dummyDataSource) {ao_setOptional(newValue, pkey: $0) }
            self.dataSource = newValue
        }
    }

    var skeletonDelegate: SkeletonCollectionDelegate? {
        get { withUnsafePointer(to: &CollectionAssociatedKeys.dummyDelegate) { return ao_get(pkey: $0) as? SkeletonCollectionDelegate } }
        set {
            withUnsafePointer(to: &CollectionAssociatedKeys.dummyDelegate) {ao_setOptional(newValue, pkey: $0) }
